...
import { createSkalStore, STORE } from './skal/store/db.js';
...
// ════════════════════════════════════════════════════════════════════
// Store tab — demo of `createSkalStore`: a reactive, persistent,
// deep-object store. One module-scope store; mutate the proxy directly,
// reads and writes are reactive, writes persist to the log engine.
// ════════════════════════════════════════════════════════════════════

const db = createSkalStore(
  {
    counter: 0,
    note: '',
    scratch: '',
  ...
  },
  {
    ...
    paths: {
      ...
      archive: { lazy: true },       // loads on first access, not at open
    },
  },
);
...
function StoreTab() {
  const ctl = db[STORE];
  const onDisk = () => ctl.backendKind() === 'native'
    || ctl.backendKind() === 'mmap' || ctl.backendKind() === 'fs';
  const engineLine = () => {
    const s = ctl.engineStats();
  ...
  return (
    <ScrollView background={BG} padding={16} gap={14} scrollbar>
      ...
      <Text
        ...
        fontSize={14}
        fontWeight={800}
        color={onDisk() ? GREEN : ORANGE}
      />
      <Text
        label={onDisk()
          ? 'Persisted — change values, quit, and re-run to verify they survive a restart.'
          : 'In-memory fallback — no writable backend, so data resets on restart.'}
        fontSize={12}
        color={SUBTLE}
      />

      {/* ── Values — mutate a plain nested object ────────────────── */}
      <Section title="Values — mutate the object directly">
        <Row gap={10}>
          <Button label="counter + 1" onClick={() => { db.counter = db.counter + 1; }} />
          <Text label={`db.counter = ${db.counter}`} fontSize={16} fontWeight={800} color={ACCENT} />
        </Row>
        <Row gap={10}>
          <Button label="toggle theme" onClick={() => {
            db.settings.theme = db.settings.theme === 'dark' ? 'light' : 'dark';
          }} />
          <Text label={`db.settings.theme = ${db.settings.theme}`} fontSize={14} fontWeight={700} color={INK} />
        </Row>
        <Text label="note — persisted; each change writes one tiny per-leaf frame" fontSize={11} color={SUBTLE} />
        <TextInput value={db.note} placeholder="persisted text…" onChange={(v) => { db.note = v; }} />
        <Text label="scratch — config persist:false, so memory only (gone on restart)" fontSize={11} color={SUBTLE} />
        <TextInput value={db.scratch} placeholder="memory-only text…" onChange={(v) => { db.scratch = v; }} />
      </Section>
      ...
      <Section title="Collection — todos (array of objects)">
        <Row gap={8}>
          ...
          <Button label="Add 100" onClick={() => batch(() => {
            for (let i = 0; i < 100; i++) {
              db.todos.push({ text: 'bulk ' + Date.now() + ' #' + i });
            }
          })} />
      ...
      <Section title="Lazy — archive (config lazy:true)">
        <Row gap={8}>
          <Button label="Add to archive" onClick={() => db.archive.push({ text: 'archived ' + Date.now() })} />
        </Row>
        <Text
          label={`${db.archive.length} records — not loaded at open; faults in from disk on first access`}
          fontSize={12}
          color={SUBTLE}
        />
      </Section>
