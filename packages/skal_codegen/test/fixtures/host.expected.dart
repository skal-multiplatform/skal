...
class _FakeSyncHost extends StatefulWidget {
  ...
  const _FakeSyncHost({required this.n, this.initial = 0});
...
class _FakeSyncHostState extends State<_FakeSyncHost> {
  ...
  Future<void> _init() async {
    try {
      final c = makeFakeViewerSync(initial: widget.initial);
      if (!mounted) {
...
Widget _build_FakeSync(NodeState n, SkalBridge bridge) {
  return _FakeSyncHost(n: n, initial: n.getCustomPropU32('initial', 0));
}
...
class _FakeAsyncHost extends StatefulWidget {
  ...
  const _FakeAsyncHost({required this.n, this.initial = 7});
...
class _FakeAsyncHostState extends State<_FakeAsyncHost> {
  ...
  Future<void> _init() async {
    try {
      final c = await makeFakeViewerAsync(initial: widget.initial);
      if (!mounted) {
        (c as dynamic).dispose();
        return;
      }
...
Widget _build_FakeAsync(NodeState n, SkalBridge bridge) {
  return _FakeAsyncHost(n: n, initial: n.getCustomPropU32('initial', 7));
}
