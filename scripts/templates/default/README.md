...
> **Requires Flutter 3.47+.** `skal_codegen` uses `analyzer ^14`, which needs
> `meta >=1.18.3`; Flutter 3.41 and older pin `meta 1.17.0` and `pub get` will
> fail to resolve with an opaque solver error.
...
Or `https://github.com/skal-multiplatform/skal/blob/main/docs/WRAPPING_PUB_PACKAGES.md` at the repo root.
